...
impl DmaChannel {
    ...
    pub fn set_client(&self, client: &'static dyn DmaClient, hardware_source: DmaHardwareSource) {
        self.client.set(client);
        self.hardware_source.set(Some(hardware_source));
    }

    /// Set this DMA channel to trigger from a hardware source
    pub fn trigger_from_hardware(&self, source: DmaHardwareSource) {
...
/// Supplies static control over all DMA channels
...
impl DmaControl {
    /// Enable the clock for all DMA channels
    pub fn enable_clock(&self) {
        self.clock.set_activity(ccm::ClockActivity::On);
    }
    /// Reset all DMA transfer control descriptors
    ///
    /// TCDs do not initialized in a defined state. The method
    /// sets all the memory to zero.
    pub fn reset_tcds(&self) {
        for tcd in &self.base.tcd {
            tcd.reset();
        }
    }
}

pub static mut DMA_CONTROL: DmaControl = DmaControl {
    base: DMA_BASE,
    clock: ccm::DMA,
};
